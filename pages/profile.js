                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex space-x-2">
                            <button onclick="window.router.currentPage.editEntry('${entry.id}', '${entry.type}')" 
                                    class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                </svg>
                            </button>
                            <button onclick="window.router.currentPage.deleteEntry('${entry.id}', '${entry.type}')" 
    ...
    async editEntry(entryId, entryType) {
        try {
            ...
            if (!entry) {
                ...
                if (entryType === 'note') {
                    entry = this.notes.find(note => String(note.id) === String(entryId));
                } else {
                    entry = this.communications.find(comm => String(comm.id) === String(entryId));
                }
                
                if (!entry) {
                    window.utils.showNotification('Entry not found - check console for details', 'error');
                    return;
                }
            }
